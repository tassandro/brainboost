...
def generate_openai_summary(transcript: str) -> str:
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Você é um assistente especializado em resumir conteúdos educacionais.",
            },
            {
                "role": "user",
                "content": f"Explique o tema da aula de forma clara e objetiva, não fale nada sobre a aula em si, seja impessoal, divida a explicação em 5 parágrafos distintos. Não use numeração, listas ou marcadores.\n{transcript}",
            },
        ],
        max_tokens=20,
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Você é um assistente que cria perguntas de múltipla escolha e indica a alternativa correta.",
                },
                {
                    "role": "user",
                    "content": f"""Crie EXATAMENTE 15 perguntas sobre o conteúdo a seguir, com alternativas numeradas (1, 2, 3, 4) e marque a resposta correta com um asterisco (*). Tome o tempo que precisar e não mencione o vídeo nas perguntas, apenas o tema ao qual o vídeo se refere
...
Conteúdo do vídeo: {transcript}""",
        ...
        for line in content.split("\n"):
            ...
            if line.startswith("P:"):
                if (
                    current_question
                    and current_question["alternatives"]
...
async def save_video_and_questions(
    db: Session,
    video_link: str,
    video_summary: str,
    transcript: str,
    ...
    try:
        ...
        for question_data in questions:
            if (
                not question_data.get("texto_questao")
                or len(question_data.get("alternatives", [])) != 4
            ):
                print(
                    f"Pergunta inválida ignorada: {question_data.get('texto_questao')}"
                )
                continue
